...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for symbol in symbols {
                ...
                (symbol["symbol"].as_str(), symbol["quoteAsset"].as_str())
            {
                if quote_asset == "USDT" {
                    ...
                    coins.push(format!("BINANCE:{}USDT", base_asset));
                }
            }
        }
    }
    ...
    let mut file = File::create("binance_usdt_markets.txt")?;
    for coin in &coins {
        writeln!(file, "{}", coin)?;
    }

    println!("Binance USDT spot piyasası coin listesi başarıyla oluşturuldu ve kaydedildi.");
    println!(
